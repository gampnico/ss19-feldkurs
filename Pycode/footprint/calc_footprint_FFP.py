...
def FFP(zm=None, z0=None, umean=None, h=None, ol=None, sigmav=None, ustar=None,
    ...
    optional inputs:
    wind_dir = wind direction in degrees (of 360) for rotation of the
    footprint
    rs       = Percentage of source area for which to provide contours,
    must be between 10% and 90%.
               Can be either a single value (e.g., "80") or a list of values
               (e.g., "[10, 20, 30]")
               Expressed either in percentages ("80") or as fractions of 1 (
               "0.8").
               Default is [10:10:80]. Set to "None" for no output of
               percentages
    nx       = Integer scalar defining the number of grid elements of the
    scaled footprint.
               Large nx results in higher spatial resolution and higher
               computing time.
               Default is 1000, nx must be >=600.
    rslayer  = Calculate footprint even if zm within roughness sublayer: set
    rslayer = 1
               Note that this only gives a rough estimate of the footprint
               as the model is not
               valid within the roughness sublayer. Default is 0 (i.e. no
               footprint for within RS).
               z0 is needed for estimation of the RS.
    crop     = Crop output area to size of the 80% footprint or the largest
    r given if crop=1
    fig      = Plot an example figure of the resulting footprint (on the
    screen): set fig = 1.
    ...
    f_ci	 = array with footprint function values of crosswind integrated
    footprint [m-1]
    x_2d	 = x-grid of 2-dimensional footprint [m], rotated if wind_dir is
    provided
    y_2d	 = y-grid of 2-dimensional footprint [m], rotated if wind_dir is
    provided
    ...
    translated to python, December 2015 Gerardo Fratini, LI-COR Biosciences
    Inc.
    version: 1.3
    ...
    fstar_ci_param = a * (xstar_ci_param - d) ** b * np.exp(
        -c / (xstar_ci_param - d))
    ...
    # Scaled sig_y*
    sigystar_param = ac * np.sqrt(
        bc * xstar_ci_param ** 2 / (1 + cc * xstar_ci_param))
    ...
    if z0 is not None:
        ...
        if np.log(zm / z0) - psi_f > 0:
            x_ci = x
            f_ci = fstar_ci_param / zm * (1. - (zm / h)) / (
                    np.log(zm / z0) - psi_f)
        else:
            x_ci_max, x_ci, f_ci, x_2d, y_2d, f_2d = None, None, None, None,\
                                                     None, None
            flag_err = 1
    else:
    ...
    for ix in range(len(f_ci)):
        f_pos[ix, :] = f_ci[ix] * 1 / (np.sqrt(2 * np.pi) * sigy[ix]) * np.exp(
            -y_pos ** 2 / (2 * sigy[ix] ** 2))
...
def get_contour_levels(f, dx, dy, rs=None):
    ...
    msf = ma.masked_array(sf, mask=(np.isnan(sf) | np.isinf(
        sf)))  # Masked array for handling potential nan
    ...
    if clevs is not None:
        ...
        for f, c in zip(fs, cs):
            ...
            if show_footprint:
                ...
                cp = ax.contour(x_2d, y_2d, f, levs, colors=cc,
                                linewidths=line_width)
            # Isopleth Labels
...
exceptions = [
    ...
    {'code': 6,
     'type': exTypes['alert'],
     'msg': 'zm (measurement height) should be above the roughness sub-layer '
            '(12.5*z0).'},
    {'code': 7,
     'type': exTypes['fatal'],
    ...
    {'code': 13,
     'type': exTypes['error'],
     'msg': 'zm (measurement height) must be above roughness sub-layer ('
            '12.5*z0).'},
    {'code': 14,
     'type': exTypes['fatal'],
